...
def get_bot_token():
    ...
    return token
TOKEN = get_bot_token()  # Get token from user input
bot = telebot.TeleBot(TOKEN)
# Global VIP list
VIP_LIST = []
CUSTOM_USERNAME = ""
# Admin ID (change to your actual admin ID)
def get_admin_id():
    admin_id = input("Please enter your Admin ID: ")
    return int(admin_id)
ADMIN_ID = get_admin_id() 

def set_custom_username():
    username = input("Ghi id tele của bạn vào khi ai đó muốn mua vip |  @name : ")
    return username
CUSTOM_USERNAME = set_custom_username()
...
def run(phone, i):
    ...
    print(f"Spam attempt {i} completed")

# /addvip command to add users to the VIP list (only the admin can use this)
...
def handle_check(message):
        ...
        # Delete waiting message after completion
    ...
    # Handler for /addvip command
@bot.message_handler(commands=['addvip'])
def addvip(message):
        user_id = message.from_user.id
        
        # Only admin can add users to the VIP list
        if user_id != ADMIN_ID:
            bot.reply_to(message, f"VUI LÒNG IB ADMIB {CUSTOM_USERNAME}")
            return
        
        # Get user ID to add to VIP list
        args = message.text.split()
        if len(args) < 2:
            bot.reply_to(message, "Usage: /addvip <user_id>")
            return
        
        vip_user_id = int(args[1])  # User ID to add to VIP list

        if vip_user_id not in VIP_LIST:
            VIP_LIST.append(vip_user_id)
            bot.reply_to(message, f"User {vip_user_id} has been added to the VIP list.")
        else:
            bot.reply_to(message, f"User {vip_user_id} is already in the VIP list.")

    # Handler for /spam command (VIP users only)
@bot.message_handler(commands=['spam'])
def spam(message):
        user_id = message.from_user.id

        # Check if user is in the VIP list
        if user_id not in VIP_LIST:
            bot.reply_to(message, f"VUI LÒNG IB ADMIB {CUSTOM_USERNAME}")
            return
        
        # Get phone number to spam
        args = message.text.split()
        if len(args) < 2:
            bot.reply_to(message, "Usage: /spam <phone>")
            return
        
        phone_number = args[1]  # Get the phone number
        count = 50  # Default spam count

        # Notify user that the spam attack is starting
        bot.reply_to(message, f"Đang Attck Với {phone_number} Sent 60 Lần ")

        # Perform the spam (but do not notify after each individual attempt)
        for i in range(1, count + 1):
            run(phone_number, i)  # Call the spam function

        # Notify user when the spam operation is completed
        bot.reply_to(message, f"Attack on {phone_number} completed 60 Lần.")
...
def help_command(message):
    help_text = """
Lệnh Bot .Spam Super 
/spam <phone_number> - SpamVip ( VIP User Only ).
/addvip <user_id> - Add a user to the VIP List (Admin Only).
/info - Show User Name ID. [ Make @Ngocdoian ]
"""
...
def start_bot():
    input_value = input(" Để Khởi Động Bot Khi Thuê tele @Ngôcdoian Cấp Thông Tin Vào Bot  :  ")
    if input_value == "cocailon":
        print("Bot is running...")
        bot.polling(none_stop=True)
    else:
        print("CÚT BUY TELE@TRUONGDEPTRAI7.")
